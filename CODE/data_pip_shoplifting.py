...
class Shoplifting_Live():
    ...
    def __init__(self):
        ...
        shoplifting_weight_path = r"D:\newFolder\Shoplifting-Detection\CODE\weight_steals\GATE_FLOW_SLOW_FAST\weights_at_epoch_shoplifting_net_model.h5"
    ...
    def make_frame_set_format(self, frame_set_src, resize=(224, 224)):
        """
    Resize frames to the given dimensions, convert them to RGB,
    and pad with two extra channels (zeros) to match the expected input shape of (224,224,5).
    """
        frame_set = []
        for frame in frame_set_src:
            ...
            # Create two extra channels (zeros)
            extra_channels = np.zeros((resize[0], resize[1], 2), dtype=frame.dtype)
            # Concatenate along the channel dimension to form a 5-channel image
            padded_frame = np.concatenate([frame, extra_channels], axis=-1)
            frame_set.append(padded_frame)
    ...
    def save_frame_set_after_pred_live_demo(self, EMS_event_path, EMS_event_frame_set, index, pred, flag, w, h):
        ...
        for frame in EMS_event_frame_set:
            # Put text in the top left corner with a smaller font
            cv2.putText(frame, "Theft alert", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            cv2.putText(frame, "Bag: %" + str(round(pred[0] * 100, 4)), (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            cv2.putText(frame, "Clothes: %" + str(round(pred[1] * 100, 4)), (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            out.write(frame)
        out.release()
    ...
    def run_Shoplifting_frames_check_live_demo(self, frame_set_format_r, ABUSE_flag):
        ...
        reports_1 = self.run_frames_check(s_1, ABUSE_flag)
        # 2
        if (len(frame_set_format_r) - 64 > 0):
            s_2 = frame_set_format_r[64:128]

            reports_2 = self.run_frames_check(s_2, ABUSE_flag)
        else:
            reports_2 = reports_1
        # 3
        if (len(frame_set_format_r) - 128 > 64):
            s_3 = frame_set_format_r[128:]
            reports_3 = self.run_frames_check(s_3, ABUSE_flag)
